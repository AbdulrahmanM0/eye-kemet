...
export default function DropFilter({ title, values, onChange }) {
    ...
    const initialSelected = values.find((v) => v.value === currentValue) ?? null
        ...
        <DropdownMenu>
            ...
            >
                {values.map((item) => (
                    ...
                    >
                        <div
                            className={`h-6 w-6 border border-gray-300 rounded flex items-center justify-center transition-all duration-200 ${selected?.id === item.id ? "bg-gold100 border-black" : ""
                                }`}
                        >
                            {selected?.id === item.id && (
                                <svg
                                    ...
                                    height="8"
                                    viewBox="0 0 11 8"
                                    className="text-[#292929] fill-current"
                                >
                                    <path
                                        fillRule="evenodd"
                                        clipRule="evenodd"
