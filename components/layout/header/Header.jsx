...
function Header({ name = false, value = false }) {
    ...
    const { handleCartToggle, cartItems } = useCart();
                ...
                <div className='px-clamp-40 sm:px-clamp-100 py-clamp-48 flex items-center justify-between'>
                    ...
                    <div className='flex gap-clamp-20 items-stretch'>
                        ...
                        <div onClick={handleCartToggle} className='cursor-pointer shrink-0 p-clamp-16 bg-full border border-gray400 rounded-[2px] flex items-center justify-center relative'>
                            ...
                            {cartItems?.length > 0 &&
                                <span class="absolute -top-3 end-0 text-white text-[8px] font-light rounded-full w-5 h-5 flex items-center justify-center"><span class="absolute inset-0 bg-[#CB4B32] rounded-full "></span><span class="absolute inset-0 bg-[#CB4B32] rounded-full shadow-lg shadow-[#CB4B32]/50 animate-ping"></span><span class="relative z-10">{cartItems.length}</span></span>
                            }
                        </div>
